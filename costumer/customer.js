...
addCustomerButt.addEventListener('click', async () => {
  await fetch(`https://${serverIP}/customers`, {
    method: 'POST',
...
async function fetchCustomers() {
  ...
  customers.forEach((customer, index) => {
    ...
    priceLevelSelect.addEventListener('change', async (e) => {
      ...
      await fetch(`https://${serverIP}/customers/${(itemId)}`, {
        method: 'PUT',
        headers: {'Content-Type': 'application/json'},
        body:
        JSON.stringify({
...
async function deleteBrand(event) {
  ...
  await fetch(`https://${serverIP}/customers/${customerId}`, {method: 'DELETE'});
  Toastify({
...
function editCustomer(event, name) {
  ...
  newInp.addEventListener('blur', async () => {
    ...
    await fetch(`https://${serverIP}/customers/${id}`, {
      method: 'PUT',
      headers: {'Content-Type': 'application/json'},
      body: JSON.stringify({
