...
addCustomerButt.addEventListener('click', async () => {
  await fetch(`http://${serverIP}:${port}/deliveries`, {
    method: 'POST',
...
async function deleteBrand(event) {
  ...
  await fetch(`http://${serverIP}:${port}/deliveries/${deliveryId}`, {method: 'DELETE'});
  Toastify({
...
function editCustomer(event, name) {
  ...
  newInp.addEventListener('blur', async () => {
    ...
    await fetch(`http://${serverIP}:${port}/deliveries/${id}`, {
      method: 'PUT',
      headers: {'Content-Type': 'application/json'},
      body: JSON.stringify({
