...
addCustomerButt.addEventListener('click', async () => {
  await fetch(`${htt}://${serverIP}${port}/workers`, {
    method: 'POST',
...
async function deleteBrand(event) {
  ...
  const response = await fetch(`${htt}://${serverIP}${port}/posinvoices`);
  const posInvoices = await response.json();
  if (posInvoices.some(invoice => invoice.workerId === workerId)) {
    Toastify({
  ...
  await fetch(`${htt}://${serverIP}${port}/workers/${workerId}`, {method: 'DELETE'});
  Toastify({
...
function editCustomer(event, name) {
  ...
  newInp.addEventListener('blur', async () => {
    ...
    await fetch(`${htt}://${serverIP}${port}/workers/${id}`, {
      method: 'PUT',
      headers: {'Content-Type': 'application/json'},
      body: JSON.stringify({
