...
addCustomerButt.addEventListener('click', async () => {
  const response  = await fetch(`${htt}://${serverIP}${port}/workers-and-get`, {
    method: 'POST',
...
async function deleteBrand(event) {
  ...
  const response = await fetch(`${htt}://${serverIP}${port}/workers-checkInv/${workerId}`, {method: 'DELETE'});
...
function editCustomer(event, name) {
  ...
  newInp.addEventListener('blur', async () => {
    ...
    const response = await fetch(`${htt}://${serverIP}${port}/workers-check/${id}?name=${name}&newVal=${newInp.value.trim().toLowerCase()}`, {
      method: 'PUT',
      headers: {'Content-Type': 'application/json'},
      body: JSON.stringify({
