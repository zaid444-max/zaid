...
addBrandButt.addEventListener('click', async () => {
  await fetch(`http://${serverIP}:${port}/category`, {
    method: 'POST',
...
async function deleteBrand(event) {
  ...
  if(items.some(item => item.category_name === name) || id === 0) {
    if (items.some(item => item.category_name === name))
      Toastify({
  ...
  await fetch(`http://${serverIP}:${port}/category/${id}`, {method: 'DELETE'});
  Toastify({
...
function editBrand(event) {
  ...
  newInp.addEventListener('blur', async () => {
    ...
    if (oldValue !== newInp.value.trim()) {
      const id = event.target.closest('tr').querySelector('.brand-id-td').innerHTML;
    await fetch(`http://${serverIP}:${port}/category/${id}`, {
      method: 'PUT',
      headers: {'Content-Type': 'application/json'},
      body: JSON.stringify({
...
saveColorButt.addEventListener('click', async function(e) {
  ...
  try {
    await fetch(`http://${serverIP}:${port}/category/${categoryId}`, {
      method: 'PUT',
      headers: {'Content-Type': 'application/json'},
      body: JSON.stringify({
    ...
    alert('please pick or write a color')
  }
})
