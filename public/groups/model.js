...
addBrandButt.addEventListener('click', async () => {
  await fetch(`http://${serverIP}:${port}/model`, {
    method: 'POST',
...
async function fetchBrands() {
  ...
  const response = await fetch(`http://${serverIP}:${port}/model?limit=${100}&search=${searchInp.value.trim()}`);
...
async function deleteBrand(event) {
  ...
  await fetch(`http://${serverIP}:${port}/model/${id}`, {method: 'DELETE'});
  Toastify({
...
function editBrand(event) {
  ...
  newInp.addEventListener('blur', async () => {
    ...
    const response = await fetch(`http://${serverIP}:${port}/model`);
    const brands = await response.json();
    ...
    if (oldValue !== newInp.value.trim()) {
      const id = event.target.closest('tr').querySelector('.brand-id-td').innerHTML;
    await fetch(`http://${serverIP}:${port}/model/${id}`, {
      method: 'PUT',
      headers: {'Content-Type': 'application/json'},
      body: JSON.stringify({
