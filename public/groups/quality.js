...
addBrandButt.addEventListener('click', async () => {
  await fetch(`http://${serverIP}:${port}/quality`, {
    method: 'POST',
...
async function fetchBrands() {
  ...
  const response = await fetch(`http://${serverIP}:${port}/quality`);
...
async function deleteBrand(event) {
  ...
  if(items.some(item => item.quality_name === name) || id === 0) {
    if (items.some(item => item.quality_name === name))
      Toastify({
  ...
  await fetch(`http://${serverIP}:${port}/quality/${id}`, {method: 'DELETE'});
  Toastify({
...
function editBrand(event) {
  ...
  newInp.addEventListener('blur', async () => {
    ...
    if (oldValue !== newInp.value.trim()) {
      const id = event.target.closest('tr').querySelector('.brand-id-td').innerHTML;
    await fetch(`http://${serverIP}:${port}/quality/${id}`, {
      method: 'PUT',
      headers: {'Content-Type': 'application/json'},
      body: JSON.stringify({
