...
async function fetchPosInvoices() {
  ...
  const response = await fetch(`${htt}://${serverIP}${port}/posinvoicesFilter?startDate=${startDateInpVal}&endDate=${endDateInpVal}&priceSelectVal=${priceSelectVal}&deliverySelectVal=${deliverySelectVal}&checkIcon=${checkIconMatch}&search=${search}&customer=${customerNameVal}&delivery=${deliveryNameVal}&worker=${workerNameVal}&limit=${limit}&searchVal=${searchVal}`);
...
window.addEventListener('beforeunload', () => {
  sessionStorage.setItem('filter_search', searchInp.value);
  //sessionStorage.setItem('filter_category', categoryInp.value);
  //sessionStorage.setItem('filter_page', pageInp.value);
});
