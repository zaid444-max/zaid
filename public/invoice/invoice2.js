...
async function fetchItems() {
  if (isAllItemFetched) return;
  const response = await fetch(`http://${serverIP}:${port}/items`);
  const items = await response.json();
  items.forEach(item => {itemListArray.push(item)});
...
async function fetchLoans() {
  const response = await fetch(`http://${serverIP}:${port}/loans`);
...
let totalItemBuy = 0;
async function fetchTargetStockInv(itemId, targetItemInvoice, itemQuantity) {
  const targetInvoice = stockInvsList.find(stockInv => stockInv.id === targetItemInvoice);
  if (targetInvoice) {
    for(const item of targetInvoice.items) {
      if (item.itemId === itemId) {
        const targettotalItemBuy = item.buyPrice * itemQuantity;
...
window.addEventListener('pageshow', async (event) => {
  if (event.persisted || window.performance.getEntriesByType("navigation")[0].type === "back_forward") {
    // Re-fetch data or re-run your function
    fetchPosInvoices();
  }
});
...
async function fetchPosInvoices() {
  ...
  const mappedItemList = itemListArray.map(item => item.id + ',' + item.brand_name + item.model_name + item.category_name + item.quality_name + (item.SKU || '') + (item.boxId || '') + (item.discription || ''));
  const filteredArray = mappedItemList.filter(item => {
    const splittedValue = searchInp.value.trim().toLowerCase().split(' ');
    return splittedValue.every(term => item.toLowerCase().includes(term))
  });
  const searchVal = searchInp.value.trim().toLowerCase();
  search = (searchInp.value.length <= 2) ? [] : filteredArray.length === 0 ? ['-1', '-1'] : filteredArray.map(item => item.split(',')[0]).slice(0, 3000);
  await fetchStockInvs();
  const response = await fetch(`http://${serverIP}:${port}/posinvoicesFilter?startDate=${startDateInpVal}&endDate=${endDateInpVal}&priceSelectVal=${priceSelectVal}&deliverySelectVal=${deliverySelectVal}&checkIcon=${checkIconMatch}&search=${search}&customer=${customerNameVal}&delivery=${deliveryNameVal}&worker=${workerNameVal}&limit=${limit}&searchVal=${searchVal}`);
  ...
  for(const [index, invoice] of posInvoices.entries()) {
    ...
    for(const item of invoice.items) {
      ...
      const itemId = parseInt(newRow.querySelector('td.id-td').querySelector('.id-span').innerHTML);
      if(!newRow.querySelector('td.target-inv-td').innerHTML) {
        Toastify({
          text: `${invoice.id} has empty stock invoice item(s)`,
          duration: 2000,
          gravity: 'top', // or 'top'
      ...
      const targetStockInvArray = newRow.querySelector('td.target-inv-td').innerHTML.match(/\d+-\d+/g).map(pair => pair.split("-").map(Number));
      for(const element of targetStockInvArray) {
        const itemQuantity = element[0];
        const targetItemInvoice = element[1];
        await fetchTargetStockInv(itemId, targetItemInvoice, itemQuantity)
      };
    };
    const itemIds = [];
    for (const item of invoice.items) {
      ...
      itemIds.push(Number(itemRow.querySelector('.id-td .id-span').innerHTML))
    }
    ...
    <td class="date-time-td" data-date="${invoice.newDate.split(',')[0]}">${invoice.newDate}</td>
    ...
    <td class="netTotal-td">${Number(invoice.netTotal).toLocaleString()}</td>
    <td class="status-td">${invoice.invStatus}</td>
    <td class="computer-td">${invoice.computerName || ''}</td>
    <td class="note-td" style="display: none;">${invoice.note}</td>
    <td class="price-level-td" style="display: none;">${invoice.priceLevel}</td>
    <td class="status-icon-td"><i class="fas fa-circle ballIcon"></i></td>
    ...
    if (loanList.some(loan => loan.customer_id === invoice.customerId) && loanList.some(loan => loan.invoiceNum === invoice.id)) {
      newRow.querySelector('.customer-td').innerHTML += " $$"
    }
  };
  updateTotal();
...
tableTbody.addEventListener('click', async (e) => {
  ...
  if (!e.target.classList.contains('delete-icon') && !e.target.classList.contains('order-td') &&  !e.target.closest('input')) {
    ...
    if (result.isConfirmed) {
      ...
      await fetch(`http://${serverIP}:${port}/posinvoices/${targetInvoiceId}`, {method: 'DELETE'})
      updateTotal();
      Toastify({
      ...
      newInp.addEventListener('blur', async function () {
        ...
        await fetch(`http://${serverIP}:${port}/posinvoices/${targettedInvoiceId}`, {
        method:'PUT',
...
async function fetchDeliveries() {
  ...
  if (!fetchedDelieveries.has(1) && !fetchedDelieveries.has(2)) {
    const response = await fetch(`http://${serverIP}:${port}/deliveries`);
...
async function fetchWorkers() {
  ...
  if (!fetchedWorkers.has(1) && !fetchedWorkers.has(2)) {
    const response = await fetch(`http://${serverIP}:${port}/workers`);
...
function searchCustomers() {
  ...
  filteredArray.forEach(span => {customDropdwon.appendChild(span)});
}
