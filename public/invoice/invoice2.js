...
async function fetchLoans() {
  const response = await fetch(`https://${serverIP}/loans`);
...
async function fetchPosInvoices() {
  ...
  const response = await fetch(`https://${serverIP}/posinvoicesFilter?startDate=${startDateInpVal}&endDate=${endDateInpVal}&priceSelectVal=${priceSelectVal}&deliverySelectVal=${deliverySelectVal}&checkIcon=${checkIconMatch}&search=${search}&customer=${customerNameVal}&delivery=${deliveryNameVal}&worker=${workerNameVal}&limit=${limit}&searchVal=${searchVal}`);
...
tableTbody.addEventListener('click', async (e) => {
  ...
  if (!e.target.classList.contains('delete-icon') && !e.target.classList.contains('order-td') &&  !e.target.closest('input')) {
    ...
    if (result.isConfirmed) {
      ...
      await fetch(`https://${serverIP}/posinvoices/${targetInvoiceId}`, {method: 'DELETE'})
      updateTotal();
      Toastify({
      ...
      newInp.addEventListener('blur', async function () {
        ...
        await fetch(`https://${serverIP}/posinvoices/${targettedInvoiceId}`, {
        method:'PUT',
...
async function fetchCustomers() {
  ...
  if (!fetchedCustomers.has(1) && !fetchedCustomers.has(2)) {
    const response = await fetch(`https://${serverIP}/customers`);
...
async function fetchWorkers() {
  ...
  if (!fetchedWorkers.has(1) && !fetchedWorkers.has(2)) {
    const response = await fetch(`https://${serverIP}/workers`);
