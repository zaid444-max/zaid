...
async function fetchCurrentInvoice() {
  const response = await fetch(`http://${serverIP}:${port}/posinvoices/${targettedInvoiceId}`);
...
async function calculateProfit() {
  for(const tr of currentInvoiceArrey) {
    ...
    for(const item of targetInv) {
      ...
      const response = await fetch(`http://${serverIP}:${port}/stockentinvs/${targetInvId}`);
    ...
    newCancelButt.addEventListener('click', async() => {
      ...
      for(const tr of currentInvoiceArrey) {
        ...
        for(const item of targetInvTdHTMLList) {
          ...
          const response = await fetch(`http://${serverIP}:${port}/stockentinvs/${targetInv}`);
          ...
          for(const item of targetStockentryInv.items) {
            if (item.itemId === itemId) {
              ...
              await fetch(`http://${serverIP}:${port}/stockentinvs/${targetInv}`, {
                method: 'PUT',
                headers: {'Content-Type': 'application/json'},
                body: JSON.stringify({
      ...
      const exists = customerLoans.some(loan => String(loan.invoiceNum).trim() === String(targettedInvoiceId).trim());
      if (exists) {
        const response = await fetch(`http://${serverIP}:${port}/oneloan/${targettedInvoiceId}`);
        const loan = await response.json();
        const updatedLoan = Number(netTotal.innerHTML.replace(/,/g, '')) - Number(loan.amount);
        await fetch(`http://${serverIP}:${port}/loans/${loan.id}`, {
          method: 'PUT',
          headers: {'Content-Type': 'application/json'},
          body: JSON.stringify({
            amount: updatedLoan
...
async function fetchItems() {
  ...
  const response = await fetch(`http://${serverIP}:${port}/itemsFilter?limit=${page}&search=${search}&categoryDivVal=${categoryInpVal}`);
  ...
  const currentInvResponse = await fetch(`http://${serverIP}:${port}/posinvoices/${targettedInvoiceId}`);
  ...
  if (targetInvoice.invStatus === 'Canceled') { // Because we do not want it when the invoice is not canceled.
    ...
    squareItems.forEach(squareItem => {
      squareItem.addEventListener('click', async (e) => {
        ...
        const respone = await fetch(`http://${serverIP}:${port}/items/${parseInt(targettedItemIdHTML)}`)
...
payButt.addEventListener('click', async () => { 
  ...
  if (currentInvoiceArrey.length !== 0) {
    ...
    firstLoop: for (const tr of currentInvoiceArrey) {
      ...
      targetInvTd.innerHTML = '';
      const response2 = await fetch(`http://${serverIP}:${port}/stockentinvs`);
        ...
        thirdLoop: for (const item of invoice.items) {
          if (item.itemId === itemId && item.lastQuantity !== 0) {
            if (item.lastQuantity >= remainingQuantity) {
              ...
              await fetch(`http://${serverIP}:${port}/stockentinvs/${invoice.id}`, {
                method:'PUT',
                headers:{'Content-Type': 'application/json'},
                body: JSON.stringify({
    ...
    const discountSpan = document.querySelector('.discount-span');
    let netTotalSpan = document.querySelector('.net-total-span');
    const response2 = await fetch(`http://${serverIP}:${port}/posinvoices/${targettedInvoiceId}`);
    ...
    const customeResponse = await fetch(`http://${serverIP}:${port}/customers/${customerInp.getAttribute('data-id')}`);
    ...
    const respone = await fetch(`http://${serverIP}:${port}/posinvoices`, {
      method: 'POST',
      headers:{'Content-Type': 'application/json'},
    ...
    if (loanCheckbox.checked) {
      const response2 = await fetch(`http://${serverIP}:${port}/loans/${customerInp.getAttribute('data-id')}`);
      ...
      if (customerLoans.some(loan => loan.invoiceNum === targettedInvoiceId)) {
        ...
        await fetch(`http://${serverIP}:${port}/loans/${targetLoan.id}`, {
          method: 'PUT',
          headers: {'Content-Type': 'application/json'},
          body: JSON.stringify({
        ...
        await fetch(`http://${serverIP}:${port}/loans`, {
          method: 'POST',
          headers: {'Content-Type': 'application/json'},
          body: JSON.stringify({
...
async function printInvoice(invoiceId, invoiceType) {
  ...
  const response = await fetch(`http://${serverIP}:${port}/customers/${customerInp.getAttribute('data-id')}`);
  const customer = await response.json();
  const deliveryRespone = await fetch(`http://${serverIP}:${port}/deliveries/${deliveryInp.getAttribute('data-id')}`);
  ...
  const loadImage = new Promise((resolve) => {
    img.onload = async function () {
      ...
      const respone2 = await fetch(`http://${serverIP}:${port}/loans/${customer.id}`);
...
removeIcon.addEventListener('click', async function () {
  ...
  const response = await fetch(`http://${serverIP}:${port}/customers/${1}`);
  const customer = await response.json();
...
async function clickSpan(e, customerName, attribute, customerInp, inputContainerDiv, removeIcon, inputType) {
  ...
  if (inputType === 'customer' || inputType === 'delivery') {
    ...
    const response = await fetch(`http://${serverIP}:${port}/customers/${attribute}`);
    const customer = await response.json();
    if (inputType === 'customer') {
      priceSelect.value = customer.priceLevel;
      priceSelect.dispatchEvent(new Event('change'));
...
async function fetchCategories() {
  const response = await fetch(`http://${serverIP}:${port}/category`);
...
async function addDiscription() {
  ...
  const searchVal = searchItemInp.value.trim();
  const categoryInpVal = categoryInp.value.replace(/\+/g, 'plus');
  const response = await fetch(`http://${serverIP}:${port}/itemsFilter?search=${searchVal}&categoryDivVal=${categoryInpVal}`);
  const items = await response.json();
