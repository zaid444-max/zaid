...
async function fetchCurrentInvoice() {
  const response = await fetch(`https://${serverIP}/posinvoices/${targettedInvoiceId}`);
...
async function calculateProfit() {
  for(const tr of currentInvoiceArrey) {
    ...
    for(const item of targetInv) {
      ...
      const response = await fetch(`https://${serverIP}/stockentinvs/${targetInvId}`);
    ...
    newCancelButt.addEventListener('click', async() => {
      ...
      for(const tr of currentInvoiceArrey) {
        ...
        for(const item of targetInvTdHTMLList) {
          ...
          const response = await fetch(`https://${serverIP}/stockentinvs/${targetInv}`);
          ...
          for(const item of targetStockentryInv.items) {
            if (item.itemId === itemId) {
              ...
              await fetch(`https://${serverIP}/stockentinvs/${targetInv}`, {
                method: 'PUT',
                headers: {'Content-Type': 'application/json'},
                body: JSON.stringify({
      ...
      const exists = customerLoans.some(loan => String(loan.invoiceNum).trim() === String(targettedInvoiceId).trim());
      if (exists) {
        const response = await fetch(`https://${serverIP}/oneloan/${targettedInvoiceId}`);
        const loan = await response.json();
        const updatedLoan = Number(netTotal.innerHTML.replace(/,/g, '')) - Number(loan.amount);
        await fetch(`https://${serverIP}/loans/${loan.id}`, {
          method: 'PUT',
          headers: {'Content-Type': 'application/json'},
          body: JSON.stringify({
            amount: updatedLoan
...
async function fetchItems() {
  ...
  const currentInvResponse = await fetch(`https://${serverIP}/posinvoices/${targettedInvoiceId}`);
...
payButt.addEventListener('click', async () => { 
  ...
  if (currentInvoiceArrey.length !== 0) {
    ...
    const discountSpan = document.querySelector('.discount-span');
    let netTotalSpan = document.querySelector('.net-total-span');
    const response2 = await fetch(`https://${serverIP}/posinvoices/${targettedInvoiceId}`);
    ...
    if (loanCheckbox.checked) {
      const response2 = await fetch(`https://${serverIP}/loans/${customerInp.getAttribute('data-id')}`);
      ...
      if (customerLoans.some(loan => loan.invoiceNum === targettedInvoiceId)) {
        ...
        await fetch(`https://${serverIP}/loans/${targetLoan.id}`, {
          method: 'PUT',
          headers: {'Content-Type': 'application/json'},
          body: JSON.stringify({
...
async function printInvoice(invoiceId, invoiceType) {
  ...
  const response = await fetch(`https://${serverIP}/customers/${customerInp.getAttribute('data-id')}`);
...
removeIcon.addEventListener('click', async function () {
  ...
  const response = await fetch(`https://${serverIP}/customers/${1}`);
  const customer = await response.json();
