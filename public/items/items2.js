...
async function fetchItems() {
  ...
  const url = `https://${serverIP}/itemsFilter?limit=${limit}&search=${searchValue}&brandDivVal=${brandDivVal}&categoryDivVal=${categoryDivVal}`
  try {
...
tableTBody.addEventListener('drop', async (e) => {
  ...
  const response = await fetch(`https://${serverIP}/items/${targetId}`);
  ...
  if (draggedItem === 'addItemButt') {
    ...
    await fetch(`https://${serverIP}/items`, {
      method: 'POST',
...
async function checkBox(event, updatedField) {
  ...
  const response = await fetch(`https://${serverIP}/items/${idTdHTML}`);
  const item = await response.json();
  await fetch(`https://${serverIP}/items/${idTdHTML}`, {
    method: 'PUT',
    headers: {
...
async function saveNewOrder() {
  ...
  fetch(`https://${serverIP}/update-order`, {
      method: "POST",
      headers: { "Content-Type": "application/json" },
  ...
  if (e.target.closest(`.${brand}-td`)) {
    ...
    newInp.addEventListener('input', async () => {
      ...
      if (brand !== 'model') {
        ...
        const response = await fetch(`https://${serverIP}/model?search=${updatedInpVal.toLocaleLowerCase()}&limit=${100}`);
...
async function editItem(e, newInp, brand, Td, idTdHTML, customDropdwonContainer) {
  ...
  Td.style.color = '';
  const response = await fetch(`https://${serverIP}/${brand}${brand === 'model' ? `?limit=${200000}` : ''}`);
  const brands = await response.json();
      ...
      if (updatedField === 'changingId') {
        ...
        for(const item of items) {
          if (item.changingId === Number(itemChanging)) {
            await fetch(`https://${serverIP}/items/${item.id}`, {
              method: 'PUT',
              headers: {'Content-Type': 'application/json'},
              body: JSON.stringify({
...
tableTBody.addEventListener('click', async (e) => {
  if (e.target.classList.contains('delete-icon')) {
    ...
    await fetch(`https://${serverIP}/items/${itemId}`, { method: 'DELETE' });
    Toastify({
...
async function exportToExcel() {
  ...
  try {
    const response = await fetch(`https://${serverIP}/export-excel`, {
      method: "POST",
      headers: {
        "Content-Type": "application/json",
      },
...
document.querySelector('.excel-file-inp').addEventListener('change', function(e) {
  ...
  reader.onload = async function(e) {
    ...
    for(const item of jsonData) {
      ...
      await fetch(`https://${serverIP}/items/${id}`, {
        method: 'PUT',
        headers: {'Content-Type': 'application/json'},
        body: JSON.stringify({
          buyPrice: buyPrice
...
async function resetItemQuantities(params) {
  ...
  for(const [index, item] of items.entries()) {
    ...
    const itemResponse = await fetch(`https://${serverIP}/items/${id}`);
