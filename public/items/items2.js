...
async function fetchItems() {
  ...
  const url = `${htt}://${serverIP}${port}/itemsFilter?limit=${limit}&search=${searchValue}&brandDivVal=${brandDivVal}&categoryDivVal=${categoryDivVal}`
  try {
...
tableTBody.addEventListener('drop', async (e) => {
  ...
  const response = await fetch(`${htt}://${serverIP}${port}/items/${targetId}`);
  ...
  if (draggedItem === 'addItemButt') {
    ...
    await fetch(`${htt}://${serverIP}${port}/items`, {
      method: 'POST',
...
async function checkBox(event, updatedField) {
  ...
  const response = await fetch(`${htt}://${serverIP}${port}/items/${idTdHTML}`);
  const item = await response.json();
...
async function saveNewOrder() {
  ...
  fetch(`${htt}://${serverIP}${port}/update-order`, {
      method: "POST",
      headers: { "Content-Type": "application/json" },
  ...
  if (e.target.closest(`.${brand}-td`)) {
    ...
    newInp.addEventListener('input', async () => {
      ...
      if (brand !== 'model') {
        ...
        const response = await fetch(`${htt}://${serverIP}${port}/model?search=${updatedInpVal.toLocaleLowerCase()}&limit=${100}`);
...
async function editItem(e, newInp, brand, Td, idTdHTML, customDropdwonContainer) {
  ...
  Td.style.color = '';
  const response = await fetch(`${htt}://${serverIP}${port}/${brand}${brand === 'model' ? `?limit=${200000}` : ''}`);
  const brands = await response.json();
      ...
      if (updatedField === 'changingId') {
        ...
        for(const item of items) {
          if (item.changingId === Number(itemChanging)) {
            await fetch(`${htt}://${serverIP}${port}/items/${item.id}`, {
              method: 'PUT',
              headers: {'Content-Type': 'application/json'},
              body: JSON.stringify({
...
tableTBody.addEventListener('click', async (e) => {
  if (e.target.classList.contains('delete-icon')) {
    ...
    await fetch(`${htt}://${serverIP}${port}/items/${itemId}`, { method: 'DELETE' });
    Toastify({
...
async function exportToExcel() {
  ...
  try {
    const response = await fetch(`${htt}://${serverIP}${port}/export-excel`, {
      method: "POST",
      headers: {
        "Content-Type": "application/json",
      },
...
async function resetItemQuantities(params) {
  ...
  for(const [index, item] of items.entries()) {
    ...
    const itemResponse = await fetch(`${htt}://${serverIP}${port}/items/${id}`);
