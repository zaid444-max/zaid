...
async function deleteLoan(event) {
  ...
  if (result.isConfirmed) {
    ...
    await fetch(`${htt}://${serverIP}${port}/loans/${loanId}`, {method: 'DELETE'});
...
broom.addEventListener('click', async function() {
  if (startIndexInp.value.trim() === '' && endIndexInp.value.trim() === '') {
    ...
    if (result1.isConfirmed) {
      await fetch(`${htt}://${serverIP}${port}/totalLoans/${currentCustomerId}`, {method: 'DELETE'});
      fetchLoans();
    }
  }
  if (startIndexInp.value.trim() !== '' && endIndexInp.value.trim() !== '') {
...
async function createPDF() {
  ...
  if (!isInvoiceInfAdded) { 
    for(const row of tableTbody.querySelectorAll('tr')) {
      ...
      const response = await fetch(`${htt}://${serverIP}${port}/posinvoices/${invoiceNum}`);
