...
async function fetchLoans() {
  ...
  const response = await fetch(`http://${serverIP}:${port}/loans/${currentCustomerId}`);
...
async function deleteLoan(event) {
  ...
  if (result.isConfirmed) {
    ...
    await fetch(`http://${serverIP}:${port}/loans/${loanId}`, {method: 'DELETE'});
...
async function editLoan(event, type, updatedField) {
  ...
  newInp.addEventListener('blur', async () => {
    ...
    await fetch(`http://${serverIP}:${port}/loans/${loanId}`, {
      method: 'PUT',
      headers: {'Content-Type': 'application/json'},
      body: JSON.stringify({
...
broom.addEventListener('click', async function() {
  if (startIndexInp.value.trim() === '' && endIndexInp.value.trim() === '') {
    ...
    if (result1.isConfirmed) {
      await fetch(`http://${serverIP}:${port}/totalLoans/${currentCustomerId}`, {method: 'DELETE'});
      fetchLoans();
    }
  }
  if (startIndexInp.value.trim() !== '' && endIndexInp.value.trim() !== '') {
...
async function createPDF() {
  ...
  if (!isInvoiceInfAdded) { 
    for(const row of tableTbody.querySelectorAll('tr')) {
      ...
      const response = await fetch(`http://${serverIP}:${port}/posinvoices/${invoiceNum}`);
