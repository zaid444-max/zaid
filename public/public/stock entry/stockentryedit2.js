...
async function createDropdown(event) {
  const stockResponse = await fetch(`${htt}://${serverIP}${port}/stockentinvs/${currentInvoiceId}`);
...
async function fetchCurrentInv() {
  const response = await fetch(`${htt}://${serverIP}${port}/stockentinvs/${currentInvoiceId}`);
...
function resetButtons() {
  fetch(`${htt}://${serverIP}${port}/stockentinvs/${currentInvoiceId}`)
  .then(response => response.json())
  ...
  if (currentIdInvoice.invStatus === 'Pending') {
    ...
    saveButt.addEventListener('click', async () => {
      ...
      for(const [index, newDiv] of newDivs.entries()) {
        ...
        const response = await fetch(`${htt}://${serverIP}${port}/items/${itemId}`);//Needed
        const targetItem = await response.json();//Needed
        /*
        const response2 = await fetch(`${htt}://${serverIP}${port}/stockentinvs/${currentInvoiceId}`);
        ...
        await fetch(`${htt}://${serverIP}${port}/items/${targetItem.id}`, {
          method:'PUT',
        ...
        const response3 = await fetch(`${htt}://${serverIP}${port}/items/${itemId}`);
    ...
    submitButt.addEventListener('click', async () => {
      ...
      if (checkboxInp.checked) {
        ...
        for(let index = 0; index < targetItems.length; index++) {
          ...
          await fetch(`${htt}://${serverIP}${port}/items/${item.itemId}`, {
            method: 'PUT',
            headers: {'Content-Type' : 'application/json'},
            body: JSON.stringify({
