...
stockentryTableTbody.addEventListener('click', async (e) => {
  if (e.target.classList.contains('delete-icon')) {
    ...
    if (result.isConfirmed) {
      const response = await fetch(`https://${serverIP}/stockentinvs/${invoiceId}`);
      try {
        ...
        await fetch(`https://${serverIP}/stockentinvs/${invoiceId}`,
          {method: 'DELETE'}
...
stockentryTableTbody.innerHTML = '';
fetch(`https://${serverIP}/stockentinvs`, { cache: 'no-store' })
