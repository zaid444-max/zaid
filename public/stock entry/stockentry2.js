...
window.addEventListener('DOMContentLoaded', () => {
  const saved = sessionStorage.getItem('stock-filters');
  if (saved) {
...
stockentryTableTbody.addEventListener('click', async (e) => {
  if (e.target.classList.contains('delete-icon')) {
    ...
    if (result.isConfirmed) {
      const response = await fetch(`${htt}://${serverIP}${port}/stockentinvs/${invoiceId}`);
      try {
        ...
        await fetch(`${htt}://${serverIP}${port}/stockentinvs/${invoiceId}`,
          {method: 'DELETE'}
...
stockentryTableTbody.innerHTML = '';
fetch(`${htt}://${serverIP}${port}/stockentinvs`, { cache: 'no-store' })
