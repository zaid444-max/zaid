...
async function createDropdown(event) {
  const stockResponse = await fetch(`https://${serverIP}/stockentinvs/${currentInvoiceId}`);
...
async function fetchCurrentInv() {
  const response = await fetch(`https://${serverIP}/stockentinvs/${currentInvoiceId}`);
...
function resetButtons() {
  fetch(`https://${serverIP}/stockentinvs/${currentInvoiceId}`)
  .then(response => response.json())
  ...
  if (currentIdInvoice.invStatus === 'Pending') {
    ...
    saveButt.addEventListener('click', async () => {
      ...
      for(const [index, newDiv] of newDivs.entries()) {
        ...
        const response = await fetch(`https://${serverIP}/items/${itemId}`);//Needed
        const targetItem = await response.json();//Needed
        /*
        const response2 = await fetch(`https://${serverIP}/stockentinvs/${currentInvoiceId}`);
        ...
        const response3 = await fetch(`https://${serverIP}/items/${itemId}`);
        ...
        */
        ...
        await fetch(`https://${serverIP}/stockentinvs/${currentInvoiceId}`, {
          method:'PUT',
          headers:{'Content-type': 'application/json'},
          body: JSON.stringify({
    ...
    submitButt.addEventListener('click', async () => {
      ...
      if (checkboxInp.checked) {
        ...
        for(let index = 0; index < targetItems.length; index++) {
          ...
          await fetch(`https://${serverIP}/items/${item.itemId}`, {
            method: 'PUT',
            headers: {'Content-Type' : 'application/json'},
            body: JSON.stringify({
